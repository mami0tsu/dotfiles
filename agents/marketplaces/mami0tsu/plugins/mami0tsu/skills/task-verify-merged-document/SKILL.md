...
- Draft PRのURL、Document変更結果、merge確認計画、merge確認の承認結果、承認済みoperation envelope、merge確認のpending operation
...
Draft PRのURLが入力された場合は、承認済みoperation envelope全体を共通のJSON digest操作へ渡し、そのoperation IDとdigestがmerge確認の承認結果およびpending operationと一致することを確認する。
Merge確認計画がoperation envelopeの反映値と同一であることを確認する。
Pull requestを取得し、host、repository、base、head、merge状態、merge commit、Document pathを承認済みのmerge確認計画と照合する。
Merge commitがmerge確認計画のbase branchから到達可能であることを確認する。
