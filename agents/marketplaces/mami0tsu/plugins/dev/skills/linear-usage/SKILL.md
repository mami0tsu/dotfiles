---
name: linear-usage
description: Linear MCP を使い、ticket-usage の共通 contract に従って issue を取得、検索、作成、更新し、担当者、状態、親子関係、block 関係を扱うときに使う。
---

# Linear Usage

[ticket contract](../ticket-usage/references/contract.md) を読み、Linear MCP の issue tool へ対応づける。
...
contract の `container`を Linear team の key、name、UUID のいずれかへ解決する。
作成時は解決済みの `team`と`title`を必須にする。
